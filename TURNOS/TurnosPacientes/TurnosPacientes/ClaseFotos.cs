...
public class ClaseFotos
{
    ...
    public static Image ConsultarFotoTurnos(string clave)
    {
        Image Resultado = null;
        byte[] mbytes;
        string Query = "";
        ...
        Query = "Select clave, foto as Photo from imgpantalla where clave='" + clave + "'";
        existereg = mycon.ExisteRegistro(Query);
        if (existereg == false) return Resultado;
        ...
        while (miRegistro.Read())
        {
            clave = miRegistro["clave"].ToString();
