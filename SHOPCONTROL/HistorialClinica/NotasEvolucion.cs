...
namespace SHOPCONTROL.HistorialClinica
{
    public partial class NotasEvolucion : Form
    {
        ...
        {
            combos.ComboDoctoresConFiltros(comboBox2, valoresg.Area_Cvdoctor);
            combos.ComboDoctoresConFiltros(comboBox1, valoresg.Area_Cvdoctor);
            ListadoDeCitas();
        ...
        public void ListadoDeCitas()
        {
            ...
            Lv.Columns.Add("Turno", 90).Tag = "STRING";
            Lv.Columns.Add("Estatus Serv", 110).Tag = "STRING";
            Lv.Columns.Add("Hora Termina", 80).Tag = "STRING";
            Lv.Columns.Add("Tiempo", 80).Tag = "STRING";

            conectorSql conecta = new conectorSql();
            string query = "Select citas.cvpaciente, citas.progresivo,citas.idturno,citas.estatusserv";
            query = query + " ,citas.horatermina,citas.hminllamada,citas.hmintermina";
            query = query + " , citas.cvdoctor,citas.numexpediente, citas.fecha, citas.horainicia, citas.estatus,citas.tipo,";
          //  query = query + " pacientes.expgineco, pacientes.expdental, pacientes.expoftamolgo,";
            query=query + " citas.nombreservicio,citas.emite,citas.recibopago, citas.nombre as nombrepac, citas.ttiempo ";
           ...
           //query = query + " left join pacientes on citas.cvpaciente=pacientes.clave "; 
            ...
            while (leer.Read())
            {
                ...
                string idturno = leer["idturno"].ToString();
                ...
                lvi.SubItems.Add(leer["estatusserv"].ToString());
        ...
        public void DetallesModifica(int index)
        {
            ...
            string idturno= Lv.Items[index].SubItems[12].Text;
            string estatusServ= Lv.Items[index].SubItems[13].Text;

            label50.Text = idturno;
            label11.Text = fecha;
            label10.Text = numticket;
            label33.Text = expediente;
            label32.Text = cvpaciente;
            ...
            label45.Text = numticket;
            label47.Text = label34.Text;
            label53.Text = comboBox1.Text.Trim();
           // combos.ComboServicio(comboBox3);
            ...
            //if (estatus == "ATENDIDO")
            //{
            //    MessageBox.Show("Se encuentra atendida la consulta clinica", "SAIMED", MessageBoxButtons.OK, MessageBoxIcon.Error);
            //    //BANDERA = true;
            //    //cargaConsecutivo();
            //    return;
            //}
            ...
            {
                MessageBox.Show("No se puede realizar la nota de evolucion se necesita el pago registrado", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Error);
                //BANDERA = true;
                //cargaConsecutivo();
                return;
            }
            ...
            if (estatus == "PAGADO")
            {
                //int indiceClick = index;
                //int indiceSIG = 0;
                //for (int i = 0; i < Lv.Items.Count; i++)
                //{
                //    string estatusVer = Lv.Items[i].SubItems[7].Text;
                //    indiceSIG = i;
                //    if (estatusVer == "PAGADO") break;
                //}
        ...
        public bool Guardar()
        {
            ...
            Query = Query + " , estatusserv='ATENDIDO'";
            Query = Query + " , horallamada='" + label52.Text.Trim() + "'";
            Query = Query + " , nomusuario='" + valoresg.USUARIOSIS + "'";
            Query = Query + " where cvpaciente='" + cvpaciente + "' and progresivo='" + numturno + "'";
            conecta.Excute(Query);


            LLamarSiguiente(doctor, label50.Text.Trim());
        ...
        public void LLamarSiguiente(string cvdoctor, string idturno)
        {
            conectorSql conecta = new conectorSql();
            string Query = "";
        ...
        private void Lv_DockChanged(object sender, EventArgs e)
        {
