...
namespace SHOPCONTROL.HistorialClinica
{
    public partial class RegistroCitas : Form
    {
       ...
       //     Lv.Columns.Add("Observa", 100).Tag = "STRING";
            ...
            string query = "Select citas.progresivo,citas.nombre,citas.ttiempo,citas.cvdoctor,citas.cvpaciente,citas.fecha,citas.horainicia,citas.estatus,citas.tipo, citas.NombreServicio, citas.emite, citas.recibopago, citas.telefono, citas.observa ";
        ...
        {
            conectorSql conecta = new conectorSql();
            string query = "Select NoExpediente,email,celular, nombre + ' ' + apaterno + ' ' + amaterno as nombrec from  pacientes where clave='" + textBox6.Text.Trim() + "'";
            SqlDataReader leer = conecta.RecordInfo(query);
            while (leer.Read())
            {
        ...
        public void DetallesModifica(int index)
        {
            ...
            if (estatus == "ATENDIDO")
            {
                Lv.Visible = true;
                panel3.Visible = false; 
                MessageBox.Show("El paciente ya se atendido", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            conectorSql conecta = new conectorSql();
            string Query = "Update citas set  estatus='CANCELADO', recibopago='0' where progresivo='" + Gnumticket + "' and fechacod='" + dateTimePicker1.Value.ToString("yyyyMMdd")+ "' and cvdoctor='" + comboBox1.SelectedValue.ToString() + "'";
            conecta.Excute(Query);
        ...
        public void GuardarRegistroServicio()
        {
            ...
            for (int i = 0; i < Lv2.Items.Count; i++)
            {
                ...
                if (existe == false)
                {
                    ...
                    Query = "Insert into DetallesPreServicio (cvpreserv,cvpaciente,cantidad,cvproducto,estatus,emitio,fecha,fechacod,Fechacita,numticket) values(";
                    Query = Query + "'" + cvpreser + "','" + cvpaciente + "','" + Lv2.Items[i].SubItems[2].Text + "','" + Lv2.Items[i].SubItems[1].Text + "','" + status + "','" + valoresg.USUARIOSIS + "','" + Fechareg.ToString("dd/MM/yyyy") + "','" + Fechareg.ToString("yyyyMMdd") + "','" + fechacita + "','" + numprogresivo + "')";
                    conecta.Excute(Query);
                    conecta.CierraConexion();
                    entroBand = true;
                }
